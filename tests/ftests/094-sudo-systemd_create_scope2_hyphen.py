...
def test(config):
    ...
    # Test 1: Verify that the expanded slice/scope was created; abort testing if it fails
    unexpected_path = os.path.join(CgroupCli.get_controller_mount_point(CONTROLLER), CGNAME)
    if os.path.exists(unexpected_path):
        result = consts.TEST_FAILED
        cause = 'Cgroup hierarchy with non-expanded slice name created: {}'.format(unexpected_path)
        return result, cause

    # Test 2: Verify that delegation succeeded
    if not Systemd.is_delegated(config, os.path.basename(EXP_CGNAME)):
        result = consts.TEST_FAILED
        cause = 'Cgroup {} is not delegated'.format(EXP_CGNAME)

    # Test 3: Verify that the controller is enabled in cgroup.subtree_control
    ...
    # Test 4: Verify that the expected directory mode is set for the cgroup
    ...
    # Test 5: Check that the expected file permissions are set for cgroup.procs
    ...
    # Test 6: Check that the expected owner ID is set for cgroup.procs
    ...
    # Test 7: Check that the expected group ID is set for cgroup.procs
...
def teardown(config, result, cause):
    ...
    try:
        CgroupCli.delete(config, None, EXP_CGNAME.partition('/')[0], recursive=True)
    except RunError as e:
        result = consts.TEST_FAILED
        tmp_cause = "Failed to remove expanded slice {}: {}".format(EXP_CGNAME, e)
        cause = '\n'.join(filter(None, [cause, tmp_cause]))
