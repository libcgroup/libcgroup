...
static void vectorize_cg(const struct cgroup * const cgrp,
			 vector<string>& cgrp_vec)
{
	const char *cgrp_name, *cgrp_cname, *value;
	int i, j;
	...
	for (i = 0; i < cgrp->index; i++) {
		for (j = 0; j < cgrp->controller[i]->index; j++) {
			...
			cgrp_vec.push_back(cgrp_cname + "+" + cgrp_name + "+" + name + "+" + value);
		}
	}
