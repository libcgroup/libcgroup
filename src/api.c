...
static int cgroup_get_cg_type(const char * const path, char * const type, size_t type_sz)
{
 ...
 *  struct cgroup *cgrp: The cgroup to assign the thread to.
 ...
 * struct cgroup *cgrp: The cgroup to assign the current thread to.
 *
...
int cgroup_attach_task(struct cgroup *cgrp)
{
	...
	error = cgroup_attach_task_pid(cgrp, tid);
...
int cgroup_get_all_controller_begin(void **handle, struct controller_data *info)
{
	...
	if (!fgets(buf, FILENAME_MAX, proc_cgrp)) {
		last_errno = errno;
		fclose(proc_cgrp);
		*handle = NULL;
		return ECGOTHER;
...
int cgroup_get_procs(char *name, char *controller, pid_t **pids, int *size)
{
	...
	cg_build_path(name, cgrp_path, controller);
	strncat(cgrp_path, "/cgroup.procs", FILENAME_MAX-strlen(cgrp_path));

	procs = fopen(cgrp_path, "r");
	if (!procs) {
		last_errno = errno;
		*pids = NULL;
		*size = 0;
